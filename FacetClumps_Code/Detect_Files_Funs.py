...
from FacetClumps import Detect_Data

def Change_pix2word(data_header, outcat, ndim, delta_LB=30, delta_V=0.167):
    """
    ...
    if 'CDELT1' in data_header and 'CDELT2' in data_header:
        ...
        size1, size2 = np.array([outcat['Size1'], outcat['Size2']])*delta_LB
        print('The size has not converted to WCS!')
        print('You may need to transform the WCS table through the Pix table by yourself.')
    ...
    if ndim == 2:
        ...
        if 'CDELT3' in data_header and 'CUNIT3' in data_header:
            if data_header['CUNIT3'] == 'm/s' or data_header['CUNIT3'] == 'm s-1':
                ...
                clump_Peaks = np.column_stack([peak1, peak2, peak3 / 1000])
                clump_Cen = np.column_stack([cen1, cen2, cen3 / 1000])
                clump_Size = np.column_stack([size1, size2, outcat['Size3'] * delta_V])
                clump_Sum = outcat['Sum'] * delta_V
                convert_to_WCS = False
        else:
...
def Table_Interface(did_table, data_header, ndim):
    ...
    if ndim == 2:
        ...
        for i in range(len(dtype)):
            ...
            if dtype[i] == 'float16':
                td_outcat[columns[i]].info.format = '.2f'
            if dtype_wcs[i] == 'float16':
                td_outcat_wcs[columns[i]].info.format = '.2f'
    elif ndim == 3:
...
def Detect(file_name, parameters, mask_name, outcat_name, outcat_wcs_name):
    ...
    path_items = outcat_name.split('/')
    exist_logic = False
    if len(path_items) > 1:
        if not os.path.exists(outcat_name[:-len(path_items[-1])]):
    ...
    if exist_logic:
        if ndim == 2:
            ...
            did_table = Detect_Data.Detect_FacetClumps_2D(RMS, Threshold, SWindow, KBins, SRecursionLB, origin_data)
        elif ndim == 3:
            did_table = Detect_Data.Detect_FacetClumps_3D(RMS, Threshold, SWindow, KBins, FwhmBeam, VeloRes,
        ...
        par_time_record.write(outcat_name[:-4] + '_FacetClumps_Detect_Record.csv', overwrite=True)
